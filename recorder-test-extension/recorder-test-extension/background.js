...
chrome.action.onClicked.addListener(async tab=>{
  ...
  if(session){await chrome.runtime.sendMessage({to:'recorder',type:'stop',reason:'user-stop'});return;}
  try{
    ...
    await chrome.runtime.sendMessage({to:'recorder',type:'start',streamId});
    await chrome.scripting.executeScript({target:{tabId:tab.id,allFrames:true},files:['monitor.js']});
  }catch(error){badge('ERR','#b3261e');await chrome.storage.local.set({lastError:error.message});if(session)await cleanup(session.tabId);session=null;await chrome.storage.session.remove('testSession');await chrome.offscreen.closeDocument().catch(()=>{});}
});
